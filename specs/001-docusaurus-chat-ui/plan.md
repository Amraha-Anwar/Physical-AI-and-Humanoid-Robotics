# P3.1 Docusaurus Chat Component: React UI and API Hookup - Implementation Plan

## Overview

This plan outlines the development of a reusable React component for the RAG chatbot, designed for seamless integration into Docusaurus MDX pages. The component will provide a two-mode query interface (standard text input and "highlight-to-query") and connect to the backend FastAPI `/api/rag/query` endpoint (P2.3) to display contextual answers.
...
*   **Selection Listener**: A global listener on the document captures text selection (`document.addEventListener('selectionchange', ...)`).
